...
export async function validateWeb4AppData(data: any): Promise<void> {
  ...
  if (data.references !== undefined && data.references !== null) {
    ...
    if (data.references.librariesOrDependencies !== undefined && data.references.librariesOrDependencies !== null) {
      checkArray(data.references.librariesOrDependencies, "references.librariesOrDependencies", false)
    ...
    data.cryptoIntegration.actions.forEach((action: any, i: number) => {
      ...
      if (action.supportedChains !== undefined && action.supportedChains !== null) {
        ...
        action.supportedChains.forEach((chainItem: any, ci: number) => {
          ...
          checkString(chainItem?.chain, `cryptoIntegration.actions[${i}].supportedChains[${ci}].chain`, true, 255)
          checkString(chainItem?.notes, `cryptoIntegration.actions[${i}].supportedChains[${ci}].notes`, false, 255)
        })
      }
    })
  }
